...
export const CreateNewRoom = (req, res) => {
  ...
  res.status(200).json({message:"New Room added Successfully",
     Room:roomData
})
...
export const BookRoom = (req,res)=>{
    ...
    let room  = roomData.filter((e)=>e.room_status ==="available" && e.room_id== roomID)
...
export const CustomerBookingDetails = (req, res) => {
    ...
    BookingRoom.forEach(booking => {
        const {roomID,customer_name,booking_date, start_time, end_time,booking_id,status  } = booking;
        ...
        customerBookings[customer_name].push({roomID,booking_date,start_time, end_time, booking_id,status  });
    });
